...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 19; i++) {
        ...
        double x = std::rand() % (800 - static_cast<int>(2 * radius)) + radius;
        double y = std::rand() % (600 - static_cast<int>(2 * radius)) + radius;
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&e) != 0) {
            if (e.type == SDL_QUIT) {
                ...
                if (mouseX - particles[selectedParticleIndex].radius < 0 ||
                    mouseX + particles[selectedParticleIndex].radius > 800 ||
                    mouseY - particles[selectedParticleIndex].radius < 0 ||
                    mouseY + particles[selectedParticleIndex].radius > 600) {
                    validPosition = false;
                }
        ...
        if (!isPaused) {
            ...
            for (size_t i = 0; i < particles.size(); i++) {
                particles[i].update(800, 600, deltaTime);
